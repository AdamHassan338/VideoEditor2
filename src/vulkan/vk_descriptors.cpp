...
std::vector<VkDescriptorSet> DescriptorAllocatorGrowable::allocate(VkDevice device, std::vector<VkDescriptorSetLayout> layouts )
{
    //get or create a pool to allocate from
    VkDescriptorPool poolToUse = get_pool(device);
    ...
    if (result == VK_ERROR_OUT_OF_POOL_MEMORY || result == VK_ERROR_FRAGMENTED_POOL) {
        ...
        VK_CHECK( vkAllocateDescriptorSets(device, &allocInfo, ds.data()));
    }
